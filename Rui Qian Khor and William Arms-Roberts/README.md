...
highest weight edge $\larrow$ nodes connected to the edges are the starting pair $\larrow$ split the pair into two sets $\larrow$ check next node's connection to each of the starting nodes $\larrow$ put the node into the set opposite the highest connection starting node $\larrow$ repeat for all other nodes
