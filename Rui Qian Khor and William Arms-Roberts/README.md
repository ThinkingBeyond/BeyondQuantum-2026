...
start at the highest weight edge
         $\downarrow$
nodes connected to the edges are the starting pair split the pair into two sets $\larrow$ check next node's connection to each of the starting nodes $\larrow$ put the node into the set opposite the highest connection starting node $\larrow$ repeat for all other nodes
\end{center}
